...
exports.handler = async (event, context) => {
  ...
  const config = loadJson(CLAWDBOT_JSON);
  const agents = config.agents?.list || [];

  const result = agents.map(agent => {
    const model = agent.model || 'unknown';
    const modelInfo = MODELS[model] || { name: model, input: 0, output: 0, tier: 'unknown' };
    return {
      id: agent.id,
      name: agent.name,
      role: agent.identity?.name || 'Unknown',
      model: model,
      ...
      groups: agent.groupChat?.mentionPatterns || []
    };
  });
